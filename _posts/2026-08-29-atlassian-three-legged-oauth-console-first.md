...
image: https://kinlane-images.s3.amazonaws.com/apievangelist/api-evangelist-images/atlassian-three-legged-oauth-console-first.png
date: 2026-08-29
author: Kin Lane
tags:
  - Onboarding
...
I keep coming back to the same wall. Every company on earth is telling me they are all in on AI, that agents are the future, that their platform is ready for an autonomous, machine-speed world. And then I go to actually wire something up, and the very first step is to open a browser, log into a dashboard, click "Create," fill out a form, and copy two strings into my clipboard by hand. There is a contradiction sitting right there at the front door, and I am tired of pretending it isn't.

This is the series where I take the thing SoundCloud quietly got right and hold it up against everyone else. SoundCloud shipped a single file you can run that opens a browser, authenticates you, registers an application, and prints `client_id` and `client_secret` to your terminal. That is what [programmatic API onboarding](https://apievangelist.com/2026/06/19/soundcloud-shows-what-programmatic-api-onboarding-should-look-like/) looks like when someone actually means it. Today I am pointing the same lens at Atlassian, the company behind Jira and Confluence Cloud, and the honest answer is: half of it is great, and the half that matters most for the agentic moment is locked behind a console.

Let me be honest about which bucket Atlassian lands in. There is no API to create an OAuth app. None. You go to the Developer Console at developer.atlassian.com, click into your profile, choose "Developer console," hit "Create," and pick "OAuth 2.0 integration." A human does this, in a browser, with a mouse. The console hands you a `client_id` and a `client_secret`, you add your Jira and Confluence scopes, and you set a callback URL by hand. That is the part no script can touch. If you wanted ten agents to each spin up their own client, you would be clicking through that wizard ten times. Atlassian also offers API tokens over at id.atlassian.com for plain Basic auth, but those are minted the same way — manually, by a person who proved they were a person.

Here is the part that is genuinely good, and it is the part I can automate. Once you have that `client_id` and `client_secret`, the rest of the OAuth 2.0 three-legged dance is clean, well-documented, and exactly the kind of thing a CLI should own. You send the user to `https://auth.atlassian.com/authorize` with `audience=api.atlassian.com`, your scopes, a `state`, and `response_type=code`. They consent. The code comes back to your callback. You exchange it at `https://auth.atlassian.com/oauth/token` for an access token. And then — this is the Atlassian-specific bit people trip on — you have to call `https://api.atlassian.com/oauth/token/accessible-resources` to find out which sites your token can actually reach. That endpoint returns the `cloudid` you need for every subsequent call to `https://api.atlassian.com/ex/jira/{cloudid}/...`. Without it you have a token and nowhere to point it.

So I wrote `atlassian-api-auth.mjs` to do everything after the console. It is one file, Node 18 and up, zero npm install. You create the app once in the Developer Console, set the callback to `http://127.0.0.1:8765/callback`, drop your `ATLASSIAN_CLIENT_ID` and `ATLASSIAN_CLIENT_SECRET` into your environment, and run it. It opens your browser, runs the full authorization-code flow on a fixed local callback, exchanges the code, then enumerates your accessible resources and prints the token and every `cloudid` it found. It is the SoundCloud ergonomics applied to the part of Atlassian that will let me apply them.
...
The script is committed in the repo at `/assets/scripts/agentic-onboarding/atlassian-api-auth.mjs` if you want to run it against your own Jira or Confluence Cloud.

Here is the hill I will happily die on. The OAuth flow is not the hard part anymore — Atlassian's is solid, and a few dozen lines of stdlib Node will carry you from consent to a working `cloudid`. The hard part is the first mile, the part where a human still has to babysit a console to mint a client. If Atlassian truly believes in the agentic moment they keep selling me, then app creation needs an API: a way for an authenticated developer or agent to register a client, declare its scopes, set a redirect URI, and get back credentials, without a single click. Until that exists, "ready for agents" means ready for agents that have a human standing behind them doing the registration by hand. I'll take the clean OAuth dance for now, because it's genuinely good. But I'm still banging my head against that console door, and I'd really like someone to open it.
