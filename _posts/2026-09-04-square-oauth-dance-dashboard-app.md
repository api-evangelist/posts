...
image: https://kinlane-images.s3.amazonaws.com/apievangelist/api-evangelist-images/square-oauth-dance-dashboard-app.png
date: 2026-09-04
author: Kin Lane
tags:
  - Onboarding
...
I keep coming back to the same wall. Every company in this industry is telling me they are all in on AI, that agents are the future, that software will soon provision and operate itself. And then they hand me a developer experience that assumes a human with a mouse, a browser, and an afternoon to spare. The contradiction is the story of this whole series, and Square is a clean, honest example of it.

Here is the thing I will say up front, because I refuse to bury it: Square does not let you create an application with an API call. There is no `POST /applications`. There is no programmatic registration. You go to the Square Developer Dashboard, you click around, you create an app by hand, and out the other side comes an application ID and an application secret. That is the ceiling. If you are building an agent that needs to stand up a fresh Square integration from nothing, the agent cannot do it. A person has to.

This is what I called [programmatic API onboarding](https://apievangelist.com/2026/06/19/soundcloud-shows-what-programmatic-api-onboarding-should-look-like/) when I wrote about SoundCloud, and Square lands in the honest middle of my buckets. The application itself is dashboard-only — that is the part that is not scriptable yet. But the OAuth dance after that? That part Square actually did well, and I am going to take what I can get.

So let me be fair about what is good here, because there is real good. Square's OAuth is clean, documented, and standard. You send the seller to `https://connect.squareup.com/oauth2/authorize` with your `client_id`, a `scope`, and a `state`. They approve. You catch the redirect, and you `POST https://connect.squareup.com/oauth2/token` with your `client_id`, `client_secret`, the `code`, and your `redirect_uri`. Back comes an `access_token`, a `refresh_token`, an `expires_at`, and — this is the nice touch — the `merchant_id` of the seller who just authorized you. Then `GET /v2/merchants/me` with that token tells you exactly whose business you are now holding credentials for. There is a sandbox host at `connect.squareupsandbox.com` that mirrors all of it, so you can build the whole flow without touching a real seller. That is a mature, predictable auth surface, and I do not say that lightly.

So I wrote the script. Same spirit as the rest of this series: one file, Node 18 and up, zero npm install, opens your browser, catches the redirect on a fixed local callback, exchanges the code, verifies the merchant, and prints your tokens to stdout. It reads `SQUARE_APPLICATION_ID` and `SQUARE_APPLICATION_SECRET` from the environment, and `SQUARE_ENV` flips the host between sandbox and production. What it does not do — what it *cannot* do — is create the app. That step still belongs to you and a web browser.

And there is one more piece of friction worth naming, because it is the exact spot where Square's reality diverges from the ideal. My callback is `http://127.0.0.1:8765/callback`, and that URL has to be registered as your redirect URL in the dashboard before any of this works. You cannot just spin up a loopback listener and go. Square will reject a redirect URI it has not seen. So even the scriptable half of this story has a manual prerequisite stapled to it. The agent can run the dance — but only after a human taught the dashboard where the agent lives.

Here is the full script. It is committed in the repo at `/assets/scripts/agentic-onboarding/square-api-auth.mjs`.
...
I will be honest about where this leaves us. Square built a payments empire on the idea that a small business should be able to start taking money in minutes, without filling out a stack of forms or waiting on a gatekeeper. That instinct — remove the friction, let people self-serve — is exactly the right instinct for the agentic moment. They already have it in their DNA. They just have not extended it all the way down to the developer surface yet.

If Square wants to fully meet this moment, the path is not mysterious. Give me an API to register an application and its redirect URLs, gated by my Square account credentials, the same way I already authenticate everything else. Let an agent provision its own integration, declare its own callback, and rotate its own secret, without a human ever opening the dashboard. The OAuth flow is already good enough that the rest of the way is short. Until then, I will run the dance, I will print the tokens, and I will keep banging my head against the one door that still only opens by hand.
