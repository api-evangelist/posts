...
image: https://kinlane-images.s3.amazonaws.com/apievangelist/api-evangelist-images/aws-cognito-has-credentials-not-front-door.png
date: 2026-07-30
author: Kin Lane
tags:
  - Onboarding
...
I keep coming back to the same wall. Every company tells me they are all in on AI, that the future is autonomous agents calling APIs on our behalf, and then they hand me an onboarding process that assumes a human with a browser, a credit card, and an afternoon to spare. The contradiction is getting harder to ignore. If an agent is supposed to go register an application and start calling your API, the very first step has to be machine-reachable. For a lot of providers, it simply is not. That is the whole reason I started reproducing SoundCloud's little `sc-api-auth.mjs` pattern across the major identity and gateway providers, to see who actually lets software do the registering.

This week it is Amazon Cognito, and Cognito is an interesting case because it sits right on the line. Cognito is an identity provider. Its entire job is issuing tokens and managing app clients with client IDs and client secrets. So you would think it would be the easiest possible target for [programmatic API onboarding](https://apievangelist.com/2026/06/19/soundcloud-shows-what-programmatic-api-onboarding-should-look-like/). It has the exact nouns I am looking for. And yet it falls squarely into what I have been calling bucket (c) in this series: cloud IAM, no public self-serve. The credentials exist, they are first-class, they are easy to mint, but the front door is locked to anyone who is not already inside the AWS account.

Here is the honest version. Cognito does not do RFC 7591 dynamic client registration. There is no anonymous `/register` endpoint where an unknown agent shows up, proves a little something, and walks away with credentials. The closest thing, and it is a genuinely good thing, is the `CreateUserPoolClient` action on the `cognito-idp` control plane. You POST to `https://cognito-idp.{region}.amazonaws.com/`, you select the action with the `X-Amz-Target: AWSCognitoIdentityProviderService.CreateUserPoolClient` header, you send a JSON body with `UserPoolId`, `ClientName`, and `GenerateSecret`, and Cognito hands you back a `UserPoolClient` object carrying `ClientId` and, if you asked for it, `ClientSecret`. That is the SoundCloud ideal, almost exactly. A client ID and a client secret, minted by an API call, for an app or an agent to then go do the OAuth2 client_credentials dance at the pool's `/oauth2/token` endpoint.

The catch is the word "almost." That POST has to be signed with AWS SigV4 using IAM credentials that already belong to the account owning the user pool. There is no browser login, no PKCE, no self-serve issuance for an outsider. You cannot bootstrap your way in. You have to already be the owner, or be handed an IAM identity by the owner, before you can create a single client. So the agent story works beautifully inside your own AWS estate and not at all across the boundary to someone else's. That is the part the marketing never says out loud.

So I wrote the script to do the piece that is actually automatable. No aws-sdk, no npm install, Node 18 standard library only. It reads `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, an optional `AWS_SESSION_TOKEN`, `AWS_REGION`, and `COGNITO_USER_POOL_ID` from the environment, signs the request itself, and creates the app client. There is one Cognito quirk worth knowing: it does not enforce unique client names and will happily create a second client with the same name rather than return a conflict, so the script lists the pool's existing clients and matches by name first, only creating when there is no match. That gives you the "already have an app, just hand it back" behavior from the SoundCloud original. Pass `--secret` and you get a confidential client with a generated secret; leave it off and you get a public client.
...
The script is committed in the repo at `/assets/scripts/agentic-onboarding/aws-cognito-api-auth.mjs`. Run it and you get `client_id=` and, with `--secret`, `client_secret=` printed to stdout, same shape as the SoundCloud original.

I will be honest about where this leaves us. For anyone building agents inside their own AWS footprint, Cognito is genuinely good. The control plane is complete, the credentials are real OAuth client IDs and secrets, and signing the call yourself is a few dozen lines of standard-library code. But the SoundCloud ideal was never about provisioning your own resources. It was about an unknown piece of software arriving at a provider it does not own and registering itself to participate. Cognito does not offer that, because Cognito is plumbing for the apps you build, not a public registration desk for the ecosystem.

If AWS wanted to fully meet the agentic moment, the move is not hard to picture. Offer a real RFC 7591 dynamic client registration endpoint on the user pool, gated by an initial access token the pool owner can hand out, so an agent can present a one-time grant and register itself without anyone wiring up IAM by hand. The standard already exists. The data model already exists. Cognito is one endpoint away from being a model citizen here, and it is a little maddening that the gap is policy rather than engineering. Until then, I will take what I can get: a clean, dependency-free way to mint a client inside my own account, and an honest note on the door about where the automation stops.
