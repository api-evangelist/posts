...
This is the table I did not expect. **Half of the providers with an agent card were added to the catalog this month**, and only 94 of the 424 were in it before July. The card did not spread through the existing catalog. It arrived attached to new companies, most of them agent-native, that either registered themselves through the open A2A registries or came in through the APIs.io add form with a card already at the well-known path. Sixty-two of them have never been touched by the catalog's own probe, because they were catalogued from the card outward rather than from the API inward. Ninety of the 424 have no OpenAPI on file at all. For those companies the agent card is the API description, and there was never a developer portal to grow it out of.
...
One more number, because it bears on what adoption means. **217 of the 424 cards give the bare host as the agent's URL.** The card says an agent exists at `https://example.com`, full stop, with no path to a JSON-RPC or HTTP+JSON endpoint behind it. Another 44 give no URL at all. Only 123 name a path that contains `a2a`, and 40 name some other path. So for more than half of the cards in the section, the artifact is a machine-readable statement of identity and skills at a well-known address, and the thing it identifies is the company's homepage. That fits what I found this morning, which is that 83 percent of these providers also run an MCP server and the card is increasingly the front door to it rather than an A2A conversation in its own right.
