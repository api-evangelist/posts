...
The agentic turn calls the bluff on any digital strategy that was never backed by real machine-readable substance. And that is exactly why the Australian banking sector — which I just scored end to end in [The State of Australian Banking APIs](https://reports.apievangelist.com/reports/state-of-australian-banking-apis/) — is such a clean specimen. The Consumer Data Right mandate, whatever its product failures, *forced the substance*.
...
I lay out the full artifact-by-artifact reading — including which two banks are already building past the mandate — in the sector report: **[The State of Australian Banking APIs](https://reports.apievangelist.com/reports/state-of-australian-banking-apis/)**. Every provider's artifacts are live in the open catalog at [APIs.io](https://apis.io), scored on the same rubric. The pipes are in. The question is who builds the actionable layer on top of them first.
