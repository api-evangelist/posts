...
image: https://kinlane-images.s3.amazonaws.com/apievangelist/api-evangelist-images/shopify-app-registration-is-dashboard-only.png
date: 2026-08-21
author: Kin Lane
tags:
  - Onboarding
...
I keep coming back to the same wall. Every company in commerce wants to talk about agents right now. Autonomous shopping, AI merchandising, bots that reconcile inventory while you sleep. And then you sit down to actually wire one of those agents into a platform and the very first step is a human clicking buttons in a dashboard. The contradiction never stops being funny to me. We are supposedly handing the keys to software, but software is not allowed to ask for the keys.

Shopify is a good place to test how serious a platform is about this, because Shopify is enormous and its API is genuinely good. So I went looking for the thing I keep wishing for: a way for a developer, or an agent acting on a developer's behalf, to register an application and walk away with credentials. The way SoundCloud lets you. The way [programmatic API onboarding](https://apievangelist.com/2026/06/19/soundcloud-shows-what-programmatic-api-onboarding-should-look-like/) should work.

I'll be honest about what I found. On Shopify, you cannot create an app with an API call. There is no public endpoint that mints you a `client_id` and `client_secret`. You make an app by hand in the Partner Dashboard, you click through the forms, and you copy the API key and secret out of the credentials screen. Custom apps are the same story from the other direction: a store owner clicks around inside their own admin to generate an Admin API access token. The registration step, the part I most want automated, is dashboard-only. Full stop.

So this provider lands in the awkward middle of the series. It is not the SoundCloud ideal where the script registers the app. But it is not a black box either. Once you have an app's key and secret, the part that turns them into something an agent can actually use, the per-store access token, is a clean, scriptable OAuth flow. That is the part worth automating, and that is the part I built.

Here is how Shopify's reality maps to the ideal. You hold an existing app's `client_id` and `client_secret`. You point a merchant's browser at `https://{shop}.myshopify.com/admin/oauth/authorize` with your client ID, the access scopes you need, a `redirect_uri`, and a `state` nonce. The merchant approves. Shopify redirects back to your callback with a `code`, plus a `shop` value and an `hmac` signature. You verify the `state` to stop CSRF, you verify the `hmac` against your app secret to prove the callback is really from Shopify, and then you do a server-to-server `POST https://{shop}.myshopify.com/admin/oauth/access_token` with your client ID, client secret, and the code. Back comes the access token. From then on every Admin API call carries it in an `X-Shopify-Access-Token` header.

What is great here is that the flow is honest and well-specified. The HMAC check is a real security boundary, not theater. Offline tokens do not expire, which is exactly what an unattended agent wants. The scopes are explicit. And because the token exchange happens off the browser, your secret never leaves your machine. Shopify did the hard parts right.

What is missing is the same thing missing almost everywhere in this series. The app itself is a manual artifact. And there is a smaller papercut on top of it: your `redirect_uri` has to be allowlisted in the app's settings before any of this works, which is one more dashboard chore. So my script bakes in a fixed local callback at `http://127.0.0.1:8765/callback`, opens the browser, catches the redirect, verifies everything, and prints the token. You still have to add that one URL to the app by hand the first time. I'll take what I can get, but I want you to see exactly where the manual seam is.

The whole thing is one file, Node 18 and up, no npm install. It reads `SHOPIFY_API_KEY`, `SHOPIFY_API_SECRET`, and `SHOP` from the environment, runs the grant, and writes the token to stdout. It is committed in the repo at `/assets/scripts/agentic-onboarding/shopify-api-auth.mjs`.
...
Here is the hill I will happily die on. If you are going to spend a keynote telling the world your platform is the home of agentic commerce, then an agent should be able to onboard the way a human can. Give me a real Dynamic Client Registration endpoint, or at least an authenticated Partner API that mints app credentials so I am not screen-scraping my own dashboard. Shopify already nailed the second half of this story. The OAuth handshake, the HMAC, the scoped tokens. It is clean and I trust it. Now close the loop on the first half, the part where the app is born, and the whole thing finally meets the moment everyone keeps insisting we are in.
