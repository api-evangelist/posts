...
image: https://kinlane-images.s3.amazonaws.com/apievangelist/api-evangelist-images/mulesoft-anypoint-and-programmatic-api-onboarding.png
date: 2026-07-08
author: Kin Lane
tags:
  - Onboarding
...
I keep coming back to the same wall. Every time I want to get a piece of software talking to an API, I find out that the part everyone treats as a formality, getting credentials, is the part that doesn't scale. You can wire up an agent to reason, plan, and call tools all day long, but somewhere in the middle of that workflow there is a human being copying a client secret out of a web console and pasting it into a config file. That is the agentic contradiction. Companies tell me they are all in on AI, and then the very first thing they ask my software to do is behave like a person clicking through a dashboard.

I wrote about [programmatic API onboarding](https://apievangelist.com/2026/06/19/soundcloud-shows-what-programmatic-api-onboarding-should-look-like/) when SoundCloud showed me what the ideal looks like: a single script opens a browser, you log in once, the script registers an application, and out come a client_id and client_secret. No portal spelunking, no support ticket, no human in the loop after that first consent. I have been retrofitting that script for the big API management platforms ever since, because I want to know who is actually ready for the moment they keep telling me they are ready for. This week it is MuleSoft Anypoint Platform.

Here is the honest assessment. Anypoint does not give you the SoundCloud flow. There is no PKCE browser login that ends in a self-serve dynamic client registration. What it gives you instead is a real management API, and a machine identity called a Connected App, and that combination is good enough to automate almost everything, just not the first step. This is the bucket I run into most: a management API plus a credential you have to mint by hand once before the automation can take over.

The way it works is layered, and you have to understand the layers or none of it makes sense. First you go into Access Management in the UI and create a Connected App with the "Application Creator" scope. That is the one manual act. That Connected App has its own client_id and client_secret, and those are machine credentials, not your personal login. You hand them to your tooling as environment variables and you are done touching a browser. From there a script mints a bearer token from the Connected App with a plain client_credentials grant against `https://anypoint.mulesoft.com/accounts/api/v2/oauth2/token`. Token in hand, you POST to the API Platform endpoint, `https://anypoint.mulesoft.com/apiplatform/repository/v2/organizations/<org>/applications`, with a name, a description, a URL, and a redirect URI, and Anypoint hands you back a brand-new client application complete with its own clientId and clientSecret. If you pass an `apiVersionId`, it even creates the contract binding that application to a specific API instance in the same call. That is genuinely good. The credential-issuing machinery is all there and all reachable over HTTP.

So I wrote the script. It is one file, Node 18 and up, no npm install, the same shape as the SoundCloud one. It reads `ANYPOINT_CLIENT_ID`, `ANYPOINT_CLIENT_SECRET`, and `ANYPOINT_ORG_ID` from the environment, mints the token, creates the application, and prints `client_id=` and `client_secret=` to stdout just like all the others in this series. It handles the already-registered case by listing your applications and returning the matching one, so re-running it is safe. The script is committed in the repo at `/assets/scripts/agentic-onboarding/mulesoft-anypoint-api-auth.mjs`.
...
```javascript
node mulesoft-anypoint-api-auth.mjs \
  --name "My Agent App" \
  --description "Agent integration" \
  --website "https://example.com"
```

What is missing is the part that matters most for the agentic moment, and I will be honest about it because pretending otherwise helps no one. The Connected App has to be created by a human in the console first. There is no flow where an agent shows up cold, consents on behalf of a user, and walks away with credentials. The bootstrap is still manual, which means the chicken-and-egg problem is just pushed back one layer rather than solved. Anypoint also splits its control planes by region, so the base URL changes for EU customers, and the management API's response envelopes are versioned enough that I had to write my listing code defensively. None of that is fatal. It is friction, and friction is what kills onboarding at scale.

If MuleSoft wants to fully meet the moment, the path is clear. Let the first Connected App be created the same way the rest of the flow already works, through an authorized OAuth consent rather than a console click. Once that one human gesture is something a script can drive, Anypoint goes from "automatable after setup" to "self-serve," and that is the whole ballgame. The plumbing is already here. I will take what I can get, and what I can get from Anypoint is a lot. I just want the last manual step to go away too.
