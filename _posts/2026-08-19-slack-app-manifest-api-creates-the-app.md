...
published: true
layout: post
title: Slack Can Actually Mint App Credentials From a Script, If You Survive the Bootstrap
image: https://kinlane-images.s3.amazonaws.com/apievangelist/api-evangelist-images/slack-app-manifest-api-creates-the-app.png
date: 2026-08-19
author: Kin Lane
tags:
  - Onboarding
...
I keep coming back to the same wall. Every company tells me they are all in on AI, that agents are the future, that the machines are coming to do the work. And then the moment I want to wire up an actual integration, they hand me a web form, a CAPTCHA, and a button I have to click with a human finger. The contradiction never gets old. You cannot build an agentic future on top of an onboarding flow that requires a person to prove they are a person.

So I have been walking through the major API providers and asking one blunt question: can a script create an application and get back credentials, the way [programmatic API onboarding](https://apievangelist.com/2026/06/19/soundcloud-shows-what-programmatic-api-onboarding-should-look-like/) should work? SoundCloud set the bar with a single-file CLI that opens a browser, does PKCE OAuth, and registers the app for you. Most vendors fall short of that bar in some embarrassing way. Slack, I'll be honest, surprised me. It is most of the way there.

Slack has an App Manifest API. You can `POST https://slack.com/api/apps.manifest.create` with a manifest JSON describing your app, and Slack creates the app and hands you back the goods in the response: `app_id`, and a `credentials` object containing `client_id`, `client_secret`, `signing_secret`, and `verification_token`. No clicking through five settings tabs. No "now copy this value and paste it over there." The thing you actually came for, the secrets, arrive in a single API call. That is the dream, and it is real here.

This is the (b) bucket in my taxonomy. It is not Dynamic Client Registration after an OAuth dance. It is a management API gated by a special token that you, the human, generate once. And that "once" is where the SoundCloud ideal cracks. To call the Manifest API you need an App Configuration Token, an `xoxe.xoxp-...` value that you create by hand at api.slack.com/apps, under "Your App Configuration Tokens," by clicking Generate Token. There is no way around that first click. An agent cannot bootstrap itself from nothing.

Now, I'll take what I can get, and Slack made the bootstrap genuinely reasonable. The config token is unique to a user and a workspace, not to an app. So one token can spin up and manage as many apps as you want in your dev workspace. You click once, then you script forever. That is a very different posture than the vendors who make you register every single application through the GUI. Slack drew the human-in-the-loop line at the workspace boundary, not at every app, and that is the right place to draw it.

The catch is that these config tokens expire after twelve hours. That sounds annoying until you see the escape hatch: `tooling.tokens.rotate`. You send it your refresh token and it returns a fresh access token and a fresh refresh token, and it needs no auth header to do it. So a long-running agent can keep itself alive indefinitely from a single human-issued seed. My script handles this. If you set `SLACK_CONFIG_REFRESH_TOKEN`, it rotates first, uses the fresh token, and prints the new pair so you can update your environment. If you only have a current `SLACK_CONFIG_TOKEN`, it just uses that. Either way the secret you paste lives in an env var, never on the command line.

Here is the part I want every other vendor to copy. Slack returns the client secret and signing secret in the create response. So many providers create the app and then make you go fish the secret out of a settings page, because they decided the API should be read-only about the very thing you need. Slack just gives it to you. The only place this falls back to the old way is `apps.manifest.update`, which does not re-return secrets, so my script is honest about that and points you to the Basic Information page if you need them again.

Here is the full script. Node 18 or newer, standard library only, no npm install, committed in the repo at `/assets/scripts/agentic-onboarding/slack-api-auth.mjs`.
...
So where does Slack land? Better than most, and genuinely close to the moment. The Manifest API plus token rotation means an agent can be handed one seed credential and then create, configure, and maintain apps on its own forever. That is real, and I want to give credit for it.

But here is a hill I will happily die on: the bootstrap should not require a browser. As long as the only way to get that first config token is a human clicking Generate Token in a web UI, Slack has not fully met the moment, it has just moved the goalposts to a place I can mostly live with. The fix is obvious. Let me obtain a scoped config token through an OAuth flow the way SoundCloud lets me obtain everything. Close the last gap, and an agent could go from zero to a working Slack app without a single human finger touching a button. Slack is one decision away from being the example everyone else should copy.
