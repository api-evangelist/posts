...
date: 2026-07-20
---
I had a conversation this week with Jakub Rozek, who spent five or six years at Stoplight and was a core contributor to Spectral and Stoplight Studio. I wanted his read on where the linter everyone governs with actually stops, because he is one of the few people who has seen it from the inside. One thing he said has been rattling around in my head ever since: there is no platform for governance. Everyone is building for documentation, or everyone is building for SDKs — and nobody is building for governance.

He is right, and once you see it you cannot unsee it. Look at the vendors. Redocly is mostly after docs. Speakeasy is mostly after SDKs. The company that owns Spectral acquired it and, as far as anyone on the outside can tell, has left it to stall — Jakub does not even have write access to the thing he helped build anymore. The forks that exist mostly optimize for one company's edge case. Governance is a slice of the market that startups do not think they need and enterprises desperately do, and it is hard to build for because every enterprise's problems are a little different. So it goes unbuilt.

### Why the linter alone was never going to be enough

The most useful part of the conversation was Jakub walking me through where Spectral runs out of road. His framing: the CLI is fine for an individual contributor who drops it into CI and moves on. But at scale, the thing you actually need is governance — and Spectral does not give you that.
...
So the answer is not a platform in the SaaS sense. It is a **composable, Git-native governance surface**: many small, open tools that each give you one honest view of the same artifacts, that you can adopt one at a time and keep forever. That is exactly what I have been building in the open under [API Commons](https://apicommons.org/tools/), and Jakub's map of the gaps lines up almost perfectly with a batch of tools I just shipped:
...
- **[Governance Waivers](https://waivers.apicommons.org)** — sanctioned, owned, expiring exceptions, now with the override/masking policy Jakub described: is this deviation allowed at all, may it be shared, is it a one-time grant?
...
Tools are only half of it. The reason we are in this spot is that the ruleset format lives inside one vendor's linter as a config file, with no separate specification and no owner willing to invest. The fix is to lift it out and make it a first-class, documented standard the community owns — which Jakub independently pointed to as the thing that unlocks a healthy ecosystem, the way Swagger's canonical implementation did for OpenAPI. That work is underway as [Spotlight Spec](https://github.com/api-commons/spotlight-spec), and I would rather do it as a community-led effort than as one more strong personality declaring "here is my approach" and walking off. If you depend on this format, you should have a seat at that table.

I am grateful to Jakub for the time and the candor. He handed me a map of exactly where the road ends, and it turns out the road ends right where the work begins. There is no platform for API governance — so let's build the open, composable one that should exist.
