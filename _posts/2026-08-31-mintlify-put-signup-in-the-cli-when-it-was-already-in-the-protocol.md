...
That signature is also now enough to get an *account*. Present it at the token endpoint with `grant_type=client_credentials` and an agent principal comes back — its own API key, its own quota, no human at any step, and `DELETE /api/v1/auth/me` to undo it in one request. Which is where the sin above stops being an anecdote and starts costing something: the reason that grant is safe to expose is that it is reachable **only** by a caller already carrying a verified AAuth identity. Anonymous is refused. An account that costs nothing to mint makes any reputation attached to it worthless — abuse the quota, take a fresh identity, repeat — so the price of one is that an agent provider on our allowlist has to vouch for you. Remove that condition and the endpoint is a sybil factory. It is not a configuration knob.

Two things I will not dress up, because a `gaps` array that only ever shrinks is marketing. There is one agent provider in that allowlist and we run it — a probe of 1,486 hosts turned up no other AAuth resource document anywhere in the catalog, so this is a worked example and not an ecosystem, and the draft it implements expires in February 2027. And an agent that can now onboard itself completely *still cannot buy anything*: the scope ceiling for an agent principal is `apis:read`, Pro data answers `402`, and machine payment is a problem I have not solved. The last human-shaped step in the chain is the one where money changes hands. That is a far more interesting place for it to be than the signup form — but it is still there, and the descriptor says so.
