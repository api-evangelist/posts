...
I would add that when an API can't be found (poor naming, poor description, no single-source etc) that a project team could decide that it doesn't exist and set about estimating the build of a new one! Even if it doesn't make it through to build (API sprawl) there is effort that has been expended on design, estimation etc. that could have been saved.<br><br>I think I will share your article every time I have the same conversations.
</div>
<br>
