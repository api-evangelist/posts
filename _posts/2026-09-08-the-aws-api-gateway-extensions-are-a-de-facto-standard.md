...
**Correction (2026-09-25):** The 107 was a count of provider listings, not companies, and most of those listings are AWS's own. The catalog profiles each AWS service as its own provider, and AWS publishes those service definitions with its gateway keys still in them. Re-counted against today's catalog, `x-amazon-apigateway-authtype` appears in 113 listings: 97 are AWS services, one is an APIs.io repository of mine, and **15 are companies outside AWS**. `x-aws-signingName` appears only in AWS's own. Outside AWS, **32 companies** publish at least one `x-amazon-apigateway-` key, and 19 of them publish `x-amazon-apigateway-integration`, the backend wiring. The argument below holds at that size. "A hundred and seven different companies" does not, and I should have separated the two before writing it.
