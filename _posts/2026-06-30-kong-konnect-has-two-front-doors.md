...
**Update (2026-09-07):** Nate Bailey, the engineering manager for the Konnect Developer Portal, wrote in with a correction to the line above: the Portal API's public OpenAPI lives at [developer.konghq.com/api/konnect/dev-portal/v3](https://developer.konghq.com/api/konnect/dev-portal/v3), and "thinly documented in public" undersold it — the v3 reference is complete and public. Verified: the page is real, and the host 404s a control path properly. One nuance survives re-probing, offered back to Kong as feedback rather than defense: the OAS renders client-side from their API-products service, and there is no directly addressable raw spec URL — every conventional path 404s — so a human can read the contract but an agent still cannot fetch it. Kong says per-portal MCP servers and agent-focused features are coming, which is exactly where an addressable spec belongs.
