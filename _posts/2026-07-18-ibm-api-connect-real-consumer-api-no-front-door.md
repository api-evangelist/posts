...
image: https://kinlane-images.s3.amazonaws.com/apievangelist/api-evangelist-images/ibm-api-connect-real-consumer-api-no-front-door.png
date: 2026-07-18
author: Kin Lane
tags:
  - Onboarding
...
I keep coming back to the same wall. Every company I talk to is "all in on AI," every roadmap has an agent on it, and yet when I actually try to get a set of credentials so that something automated can call an API, I am back in a web form, clicking through a portal, proving I am a human who deserves a client_id. That contradiction does not get less absurd the more times I hit it. If you want machines to consume your APIs, the act of becoming a consumer has to be something a machine can do.

That is why I have been retracing SoundCloud's example across the big gateway and management vendors. SoundCloud showed what [programmatic API onboarding](https://apievangelist.com/2026/06/19/soundcloud-shows-what-programmatic-api-onboarding-should-look-like/) can look like: one small script opens a browser, you log in once, and it hands you back a client_id and client_secret. No ticket, no sales call, no copy-pasting out of a dashboard. This week I pointed that same lens at IBM API Connect, and the answer is more interesting than a simple yes or no.

Here is the good news, and I want to be fair because it is genuinely good. API Connect has a real consumer API. The Developer Portal exposes a REST surface, the `consumer-api`, that lets you do the things a human does in the portal: create an application in a consumer organization, read back its credentials, and subscribe that application to a published product plan. You `POST /consumer-api/orgs/{org}/apps`, and the response carries the client_id and a client_secret. You then `POST .../apps/{appID}/subscriptions` with a product and a plan name to wire it up. None of this is a hack. IBM documents it, and it does exactly what you would hope. So the raw capability that SoundCloud built is, in fact, present here.

Now the honest part. This is not bucket (a). There is no RFC 7591 dynamic client registration, and there is no single IBM-wide "log in with IBM" button that drops you into a credential. This lands squarely in bucket (b), the management-API-plus-a-token model, with a heavy dose of the friction that makes bucket (c) feel familiar. Every call to the consumer API is scoped to exactly one provider organization and one catalog, and you have to announce that scope on every request with an `X-IBM-Consumer-Context: <provider org>.<catalog>` header. Get it wrong and the gateway cannot even route you. Worse, the bearer token is not minted by some global identity service. It comes from that specific catalog's portal OIDC identity provider, with a realm string like `consumer:{porg}:{catalog}/{catalog}-idp`. And to mint it via the documented password grant, you already need a client_id and client_secret for a "toolkit" application in that catalog. Read that again: to programmatically get credentials, you first need credentials.

That chicken-and-egg is the whole story. SoundCloud's script can ship a bundled client and walk a brand-new user from zero to a working app. With API Connect, the generic, copy-paste-across-any-catalog experience does not exist, because the identity, the realm, and the routing context are all local to a single provider org and catalog. There is no universal endpoint to hardcode. That is a defensible architecture for an enterprise gateway where every provider runs their own portal, but it means the SoundCloud ideal is out of reach without a human first standing up the scope you are scripting against.

So I did what I could do honestly, which is automate everything downstream of that one human-shaped step. My script reads `APIC_PORTAL_URL`, `APIC_CONSUMER_ORG`, `APIC_CATALOG`, and `APIC_PROVIDER_ORG` from the environment, takes a bearer token in `APIC_TOKEN`, and goes straight at the consumer API: create the app, read back client_id and client_secret, optionally subscribe to a plan, and print the credentials to stdout. I documented, in the file, the two real ways a person actually gets that token, and I added a `--bootstrap-token-username/--password` path that mints one for you through `POST /consumer-api/token` when you already hold a toolkit app's credentials. It handles the already-exists case by finding and reusing the app, and it is loud about the fact that the secret only comes back at creation time. No browser dance, because there is no single sign-in to dance with. The script is committed in the repo at `/assets/scripts/agentic-onboarding/ibm-api-connect-api-auth.mjs`.
...
I'll take what I can get, and this is more than I usually get. But if IBM wants API Connect to meet the agentic moment, the gap is clear. Give me one well-known discovery endpoint that resolves a catalog to its consumer API and its OIDC realm, and give me a true dynamic client registration flow so a brand-new consumer can bootstrap a first credential without already holding one. The consumer API is right there. It just needs a front door an agent can walk through on its own.
